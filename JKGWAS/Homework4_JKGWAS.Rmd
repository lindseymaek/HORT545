...
# myGD=mdp_numeric <- read.delim("~/Desktop/Data/mdp_numeric.txt")
# myGM=mdp_SNP_information <- read.delim("~/Desktop/Data/mdp_SNP_information.txt", comment.char="#")
# myCV=CROPS545_Covariates <- read.delim("~/Desktop/Data/CROPS545_Covariates.txt")

## Phenotype simulation

set.seed(99);
mySim=GAPIT.Phenotype.Simulation(GD=X.num,GM=SNP,h2=.75,NQTN=10, effectunit =.95,QTNDist="normal",CV=CV);

myGAPIT_BLINK = GAPIT(Y=mySim$Y,GD=X.num,GM=SNP, CV=CV, PCA.total = 3, QTN.position = mySim$QTN.position, model = "Blink",file.output = F)      ## GWAS with Blink

GWAS_result = myGAPIT_BLINK$GWAS     ##Extracting the GWAS result from Blink
GWAS_p_order = GWAS_result[order(GWAS_result$P.value, decreasing = FALSE),]  ## Sorting QTNs by their p-values
GWAS_p_order_Bonferroni = p.adjust(GWAS_p_order$P.value,method = "bonferroni") ## bonferroni multiple test corrections
pass_threshold <- GWAS_p_order[GWAS_p_order_Bonferroni < 0.05, ]   ## SNPs that pass 5% threshold after bonferroni correction
pass_threshold       ## SNPs that pass 5% threshold
```
